...
export async function updateVendorProfile(userId, payload) {
  const admin = getSupabaseAdmin();
  const { data: existing, error: exErr } = await admin.from("vendors").select("id, category").eq("user_id", userId).maybeSingle();
  if (exErr) return { data: null, error: exErr };
  ...
  if (payload.phone !== undefined) {
    patch.phone = payload.phone != null ? String(payload.phone).trim() : null;
    // Phone edits invalidate previous OTP verification status.
