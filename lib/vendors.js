...
import { mapVenueHighlightsFromRow, validateVenueHighlightsPayload } from "./venueHighlights";
...
function isMissingVenueHighlightColumnError(err) {
  const m = String(err?.message ?? err ?? "").toLowerCase();
  if (!m.includes("column")) return false;
  const refers = VENUE_HIGHLIGHT_DB_KEYS.some(
    (k) => m.includes(`"${k}"`) || m.includes(`'${k}'`) || new RegExp(`\\b${k}\\b`).test(m),
  );
  if (!refers) return false;
  return m.includes("schema cache") || m.includes("could not find") || m.includes("does not exist");
}
...
/** @param {unknown} err */
function getMissingVenueHighlightColumnsFromError(err) {
  const m = String(err?.message ?? err ?? "").toLowerCase();
  if (!isMissingVenueHighlightColumnError(err)) return [];
  return VENUE_HIGHLIGHT_DB_KEYS.filter(
    (k) => m.includes(`"${k}"`) || m.includes(`'${k}'`) || new RegExp(`\\b${k}\\b`).test(m),
  );
}

function stripVenueHighlightFields(obj, keys = VENUE_HIGHLIGHT_DB_KEYS) {
  const next = { ...obj };
...
export async function updateVendorStatus(id, status) {
  ...
  const admin = getSupabaseAdmin();
  const { data: existing, error: exErr } = await admin.from("vendors").select("category").eq("id", id).maybeSingle();
  if (exErr) return { data: null, error: exErr };

  /** @type {Record<string, unknown>} */
  const patch = { status };
  if (status === "approved" && String(existing?.category || "").trim() === "Venue") {
    patch.verified_venue = true;
  }

  let attempt = { ...patch };
  for (let i = 0; i < 4; i++) {
    const { data, error } = await admin.from("vendors").update(attempt).eq("id", id).select("*").single();
    if (!error) return { data: mapVendorRow(data), error: null };
    if (isMissingVenueHighlightColumnError(error) && "verified_venue" in attempt) {
      const missing = getMissingVenueHighlightColumnsFromError(error);
      attempt = stripVenueHighlightFields(attempt, missing.length ? missing : ["verified_venue"]);
      continue;
    }
    return { data: null, error };
  }
  return { data: null, error: new Error("Could not update vendor status") };
}
 ...
 */
/**
 * Admin-only: featured / verified badges for venue listings.
 */
export async function updateVenueAdminFlagsByAdmin(id, { featuredVenue, verifiedVenue }) {
  const admin = getSupabaseAdmin();
  const { data: existing, error: exErr } = await admin.from("vendors").select("id, category").eq("id", id).maybeSingle();
  if (exErr) return { data: null, error: exErr };
  if (!existing) return { data: null, error: new Error("Vendor not found") };
  if (String(existing.category || "").trim() !== "Venue") {
    return { data: null, error: new Error("Admin venue flags apply to Venue category only") };
  }

  const patch = {};
  if (featuredVenue !== undefined) patch.featured_venue = Boolean(featuredVenue);
  if (verifiedVenue !== undefined) patch.verified_venue = Boolean(verifiedVenue);
  if (Object.keys(patch).length === 0) return getVendorById(id);

  let attempt = { ...patch };
  for (let i = 0; i < 4; i++) {
    const { data, error } = await admin.from("vendors").update(attempt).eq("id", id).select("*").single();
    if (!error) return { data: mapVendorRow(data), error: null };
    if (isMissingVenueHighlightColumnError(error)) {
      const missing = getMissingVenueHighlightColumnsFromError(error);
      if (missing.length) {
        attempt = stripVenueHighlightFields(attempt, missing);
        if (Object.keys(attempt).length === 0) {
          return { data: null, error: new Error("Venue highlight columns are not migrated yet") };
        }
        continue;
      }
    }
  ...
  return { data: null, error: new Error("Could not update venue admin flags") };
}
...
/**
 * True when the vendor has a WhatsApp-verified phone that matches the number being saved.
 * Used to allow profile PATCH without a fresh OTP session when the phone is unchanged.
 * @param {string} userId
 * @param {string | null | undefined} [phoneInPayload]
 */
export async function vendorHasActiveVerifiedPhone(userId, phoneInPayload) {
  const admin = getSupabaseAdmin();
  const { data, error } = await admin
    .from("vendors")
    .select("phone, phone_verified_at, phone_verified_e164")
    .eq("user_id", userId)
    .maybeSingle();
  if (error || !data?.phone_verified_at || !data?.phone_verified_e164) return false;

  const verifiedDigits = normalizeWhatsAppRecipientDigits(data.phone_verified_e164);
  if (!verifiedDigits) return false;

  const phoneRaw =
    phoneInPayload !== undefined && phoneInPayload !== null
      ? String(phoneInPayload).trim()
      : String(data.phone || "").trim();
  const payloadDigits = normalizeWhatsAppRecipientDigits(phoneRaw);
  return Boolean(payloadDigits && payloadDigits === verifiedDigits);
}
...
export async function updateVendorProfile(userId, payload) {
  ...
  if (payload.venueHighlights !== undefined) {
    if (effectiveCategory !== "Venue") {
      return { data: null, error: new Error("venueHighlights is only valid for Venue category") };
    }
    const v = validateVenueHighlightsPayload(payload.venueHighlights);
    if (!v.ok) return { data: null, error: new Error(v.error) };
    Object.assign(patch, v.data);
    if (v.data.guest_capacity != null) {
      patch.capacity = `${v.data.guest_capacity} guests`;
    }
  }
  ...
  if (effectiveCategory !== "Venue") {
    patch.venue_details = [];
    patch.venue_type = null;
    patch.years_in_business = null;
    patch.guest_capacity = null;
    patch.dining_capacity = null;
    patch.parking_capacity = null;
    patch.air_conditioned = false;
    patch.stage_available = false;
    patch.wheelchair_accessible = false;
    patch.suitable_for = [];
  }
  if (effectiveCategory !== "Photographer") {
  ...
  let venueHighlightsDropped = false;

  /** Enough retries to strip each optional column individually (11 highlight keys + other optional fields). */
  const MAX_UPDATE_RETRIES = 24;
  /** @type {unknown} */
  let lastError = null;
  ...
  for (let i = 0; i < MAX_UPDATE_RETRIES; i++) {
    ...
    if (isMissingVenueHighlightColumnError(error) && VENUE_HIGHLIGHT_DB_KEYS.some((k) => k in attempt)) {
      const missing = getMissingVenueHighlightColumnsFromError(error);
      if (missing.length) {
        attempt = stripVenueHighlightFields(attempt, missing);
        venueHighlightsDropped = true;
        continue;
      }
    }
    return { data: null, error: lastError || new Error("Could not update vendor profile") };
  }

  const lastMsg = lastError?.message ? String(lastError.message) : "Could not update vendor profile";
  return {
    data: null,
    error: new Error(
      `${lastMsg} (retried ${MAX_UPDATE_RETRIES} times — if highlight fields are involved, run supabase/migrations/016_venue_highlights.sql and 017_venue_suitable_for.sql in Supabase SQL editor).`,
    ),
  };
}
...
/**
 * Gallery images after the hero (index 0), preserving DB array order as sort_order.
 * @param {object} row - vendors row with `gallery_images`
 * @returns {{ url: string; responsive: ReturnType<typeof parseResponsiveImageField>; sort_order: number }[]}
 */
export function getGalleryImagesAfterHeroFromRow(row) {
  const raw = row?.gallery_images;
  ...
  return raw
    ...
    .map((entry, index) => {
      ...
      const responsive = parseResponsiveImageField(trimmed);
      const url = responsive?.large || trimmed;
      if (!url) return null;
      return { url, responsive, sort_order: index + 1 };
    })
    .filter(Boolean);
}

/** Approved vendor — remaining gallery images (skips hero at sort index 0). */
export async function getPublicVenueGalleryImagesAfterHero(id) {
  const admin = getSupabaseAdmin();
  const { data, error } = await admin
    .from("vendors")
  ...
  return { data: getGalleryImagesAfterHeroFromRow(data), error: null };
}

/** Approved venue ids for ISR paths (no `views` column — newest approved venues first). */
export async function getApprovedVenueIdsForStaticPaths(limit = 50) {
  const admin = getSupabaseAdmin();
  const { data, error } = await admin
    .from("vendors")
    .select("id")
